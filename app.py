...
def load_question_names():
    ...
    for name in names:
        parts = name.split(". ")
        if len(parts) > 1:
            question_names.append(parts[1].strip())
        else:
            print("Invalid format for line:", name)
    return question_names


Qnames = load_question_names()
    ...
    for term in q_terms:
        ...
        for doc_index in potential_docs:
            ...
            ans.append({"Question Link": Qlink[int(
                doc_index) - 1][:-2], "Score": potential_docs[doc_index], "Question Name": Qnames[int(doc_index)-1]})
    return ans
...
def home():
    ...
    if form.validate_on_submit():
        ...
        q_terms = [term.lower() for term in query.strip().split()]
        results = calc_docs_sorted_order(q_terms)[:24:]
